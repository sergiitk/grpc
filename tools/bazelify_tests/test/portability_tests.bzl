...
def generate_run_tests_portability_tests(name):
    ...
    for language in ["c", "c++"]:
        compiler_configs = [
            ...
            # With gcc-8, building gRPC buildtests_cxx targer either times out,
            # or fails with collect2: fatal error: ld terminated with signal 9.
            # We investigated it this issue as an OOM issue, but increasing
            # memory does not help. Increasing timeouts doesn't help either.
            # Debugging PR: https://github.com/grpc/grpc/pull/41028
            # Since gcc is expected to be out of support, we'll be disabling
            # tests build, and only build grpc++ make target. See build_cxx.sh.
