...
class KubernetesBaseRunner(base_runner.BaseRunner):
    ...
    def __init__(
        self, k8s_namespace, namespace_template=None, reuse_namespace=False
    ):
        super().__init__()
    ...
    def _create_from_template(self, template_name, **kwargs) -> object:
        ...
        if next(manifests, False):
            raise _RunnerError(
                "Exactly one document expected in manifest " f"{template_file}"
            )
    ...
    def _create_service(self, template, **kwargs) -> k8s.V1Service:
        ...
        if not isinstance(service, k8s.V1Service):
            raise _RunnerError(
                "Expected V1Service to be created " f"from manifest {template}"
            )
    ...
    def _logs_explorer_link(
        ...
        *,
        ...
        gcp_project: str,
        gcp_ui_url: str,
        end_delta: Optional[_timedelta] = None,
    ) -> None:
