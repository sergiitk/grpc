    ...
    def client_cmd(self, args):
        ...
        for arg in args:
            ...
            if port:
                ...
                cmdline = (
                    'pod install && xcodebuild -workspace Tests.xcworkspace -scheme InteropTestsLocalSSL -destination name="iPhone 6" HOST_PORT_LOCALSSL=localhost:%s test'
                    % portnum
                )
                return [cmdline]
...
argp.add_argument(
    ...
    nargs="+",
    help="Credential types to test in the cloud_to_prod setup. Default is to test with all creds types possible.",
)
argp.add_argument(
