...
for attr in attrs:
    ...
    if attr["default"] not in DEFAULTS:
        print(
            "invalid default for experiment %s: %r"
    ...
    if check_dates:
        if expiry < today:
            print(
                "experiment %s expired on %s" % (attr["name"], attr["expiry"])
            )
            error = True
        if expiry > two_quarters_from_now:
            print(
                "experiment %s expires far in the future on %s"
...
with open("src/core/lib/experiments/experiments.h", "w") as H:
    ...
    for i, attr in enumerate(attrs):
        print(
            "#define GRPC_EXPERIMENT_IS_INCLUDED_%s" % attr["name"].upper(),
            file=H,
        )
        print(
...
with open("src/core/lib/experiments/experiments.cc", "w") as C:
    ...
    if (
        "kDefaultForDebugOnly" in have_defaults
        or "kDefaultForReleaseOnly" in have_defaults
    ):
    ...
    for attr in attrs:
        print(
            ...
            % (
                ...
                "true"
                if attr.get("allow_in_fuzzing_config", True)
                else "false",
            ),
            file=C,
        )
...
with open("bazel/experiments.bzl", "w") as B:
    ...
    print(
        (
            '"""Dictionary of tags to experiments so we know when to test'
